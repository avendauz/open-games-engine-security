...
Compositional game theory intuitively represent games as processes. When reading a string diagram, we consider each box a morphism and the incoming and outgoing wires as objects in the underlying categ
...
Testing some code, we want ot examine 
\begin{code}
	PayoffReader :: Reader A Double
\end{code}

And some inline code |f :: X -> Y|
