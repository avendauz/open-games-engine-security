...
Haskell is a statically-typed, functional programming language. We also have the ability to define custom data types using the keyword \ensuremath{\mathbf{data}} to represent actions or outcomes. 
\begin{mdframed}
...
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\mathbf{data}\;\Conid{Honeypot}\mathrel{=}\Conid{Hinteract}\mid \Conid{Linteract}\mid \Conid{Normal}\;\mathbf{deriving}\;(\Conid{Eq},\Conid{Show}){}\<[E]%
\ColumnHook
\end{hscode}\resethooks

where \ensuremath{\Conid{Honeypot}} has three constructors corresponding to the three states. Here there are no type variables. The \ensuremath{\mathbf{deriving}} keyword specifies which type classes this data type is an instance of, in this case two type classes \ensuremath{(\Conid{Eq},\Conid{Show})} imported from \ensuremath{\Conid{Prelude}}. This gives our data type operations for equality to compare instances of \ensuremath{\Conid{Honeypot}} and the ability to read data in the console as \ensuremath{\Conid{String}}. 
\end{example}
...
The \textit{open-game-engine} provides a helper function \ensuremath{\Varid{playDeterministically}} to assign any type a probability of 1. 

\begin{hscode}\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{definitelyHoneypot}\mathrel{=}\Varid{playDeterministically}\;\Conid{Honeypot}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
\end{example}
\end{mdframed}
...
In the classic game-theoretic sense, this strategy assigns some probabilistic action of type \ensuremath{\Varid{a}} for every observed type \ensuremath{\Varid{s}}. 

\begin{mdframed}
\begin{example}[Honeypot allocation randomization]
Say we want to describe a defensive strategy that deploys a high interaction honeypot 75\% of the time, a low-interaction honeypot 25\% of the time, and doesn't activate a honeypot 10\% of the time. We can use the helper function \ensuremath{\Varid{distFromList}} and the data type in Example \ref{example:userTypes} as our set of actions. Assuming we deploy this defensive strategy statically, without any observations, we use the type \ensuremath{()} to describe no input.
...
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{defensiveStrategy}\mathbin{::}\Conid{Kleisli}\;\Conid{Stochastic}\;()\;\Conid{Honeypot}{}\<[E]%
\\
\>[B]{}\Varid{defensiveStrategy}\mathrel{=}\Conid{Kleisli}\mathbin{\$}\Varid{const}\mathbin{\$}{}\<[E]%
\\
\>[B]{}\hsindent{7}{}\<[7]%
\>[7]{}\Varid{distFromList}\;{}\<[E]%
\\
\>[7]{}\hsindent{3}{}\<[10]%
\>[10]{}[\mskip1.5mu (\Conid{HighInteraction},\mathrm{0.75}),{}\<[E]%
\\
\>[7]{}\hsindent{3}{}\<[10]%
\>[10]{}(\Conid{LowInteraction},\mathrm{0.25}),{}\<[E]%
\\
\>[7]{}\hsindent{3}{}\<[10]%
\>[10]{}(\Conid{Normal},\mathrm{0.10})\mskip1.5mu]{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[4]{}\mathbf{data}\;\Conid{StochasticOptic}\;\Varid{s}\;\Varid{t}\;\Varid{a}\;\Varid{b}\;\mathbf{where}{}\<[E]%
\\
\>[4]{}\hsindent{3}{}\<[7]%
\>[7]{}\Conid{StochasticOptic}\mathbin{::}{}\<[27]%
\>[27]{}(\Varid{s}\to \Conid{Stochastic}\;(\Varid{z},\Varid{a})){}\<[E]%
\\
\>[27]{}\hsindent{1}{}\<[28]%
\>[28]{}\to (\Varid{z}\to \Varid{b}\to \Conid{Stochastic}\;\Varid{t}){}\<[E]%
\\
\>[27]{}\hsindent{1}{}\<[28]%
\>[28]{}\to \Conid{StochasticOptic}\;\Varid{s}\;\Varid{t}\;\Varid{a}\;\Varid{b}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
\end{minipage}


Note the implicit quantification \ensuremath{\Varid{forall}\;\Varid{z}} as part of this data type, which was explicitly given as $\Theta$ in Definition \ref{Definition:Coends}. We retain the value of \ensuremath{\Varid{z}}, the "residual" to be used in computing the backwards pass, which is given as the second argument given to this data constructor. 

\begin{mdframed}
\begin{example}[Forward probabilistic function]\label{example:forward}
We can describe a class of stochastic optics \ensuremath{\Conid{StochasticOptic}\;\Varid{s}\;()\;\Varid{a}\;()} which performs a probabilistic computation and disregards any contravariantly flowing information. So we can lift any morphism \mbox{$f: S \rightarrow \Prob(A)$} of $\KlD$ to an optic using the following Haskell function: 
\begin{hscode}\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{4}{@{}>{\hspre}l<{\hspost}@{}}%
\column{7}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{liftStochastic}\mathbin{::}(\Varid{s}\to \Conid{Stochastic}\;\Varid{a})\to \Conid{StochasticOptic}\;\Varid{s}\;()\;\Varid{a}\;(){}\<[E]%
\\
\>[B]{}\Varid{liftStochastic}\;\Varid{f}\mathrel{=}{}\<[E]%
\\
\>[B]{}\hsindent{4}{}\<[4]%
\>[4]{}\Conid{StochasticOptic}\;{}\<[E]%
\\
\>[4]{}\hsindent{3}{}\<[7]%
\>[7]{}(\lambda \Varid{x}\to \mathbf{do}\;\{\mskip1.5mu \Varid{y}\leftarrow \Varid{f}\;\Varid{x};\Varid{return}\;((),\Varid{y})\mskip1.5mu\})\;{}\<[E]%
\\
\>[4]{}\hsindent{3}{}\<[7]%
\>[7]{}(\lambda ()\;()\to \Varid{return}\;()){}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
which we can parameterize with the datatype \ensuremath{\Conid{StochasticOptic}} to get our desired operations for "stochastic optics," located in \ensuremath{\Conid{\Conid{OpenGames}.\Conid{Engine}.OpticClass}}. Optic sequential and parallel composition will form the basis of how agent's game-theoretic behaviour compose, with extra details about how to interpret their local contexts given in the next section. We'll also use this opportunity to show how these optics plug together using the type parameters. First, note the type of \ensuremath{(\mathbin{>>>>})} enforces that the output type \ensuremath{\Varid{a}} and the contravariant input type \ensuremath{\Varid{b}} of the first optic correspond to the input type and the contravariant output type of the second optic. Similarly, the type of \ensuremath{(\mathbin{\&\&\&\&})} enforces that each of the input and output types are tupled together. Most IDEs will give a pop-up corresponding to the \textbf{ghc} typechecker to help a user line these types up when usingthese operations, or a direct error message will be given when trying to compile in \textbf{ghci}.  
...
Given a strategy from the set of strategies from (1) that assigns an outcome of type \ensuremath{\Conid{Stochastic}\;\Varid{a}} representing probability distribution over actions for each observation of type \ensuremath{\Varid{s}}, we generate a stochastic optic that is "preloaded" with the behaviour of this strategy. This optic \ensuremath{\Conid{StochasticOptic}\;\Varid{s}\;\Varid{t}\;\Varid{a}\;\Varid{b}} is open in the sense that it's ready to respond by choosing an action according to its local \textit{context} \footnote{see the following section, \ref{sec:contexts}}. So we can describe a family of optics indexed by behavioural strategies using the Haskell function:
\begin{hscode}\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{7}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{play}\mathbin{:}\Conid{Kleisli}\;\Conid{Stochastic}\;\Varid{s}\;\Varid{a}\to \Conid{StochasticOptic}\;\Varid{s}\;()\;\Varid{a}\;\Conid{Double}{}\<[E]%
\\
\>[B]{}\Varid{play}\;\Varid{strategy}\mathrel{=}\Conid{StochasticOptic}{}\<[E]%
\\
\>[B]{}\hsindent{7}{}\<[7]%
\>[7]{}(\lambda \Varid{s}\to \mathbf{do}\;\{\mskip1.5mu \Varid{y}\leftarrow \Varid{runKleisli}\;\Varid{strategy}\;\Varid{s};\Varid{return}\;((),\Varid{a})\mskip1.5mu\}){}\<[E]%
\\
\>[B]{}\hsindent{7}{}\<[7]%
\>[7]{}(\lambda ()\;\anonymous \to \Varid{return}\;()){}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
\column{7}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[4]{}\mathbf{data}\;\Conid{OpenGame}\;\Conid{StochasticOptic}\;\Conid{StochasticContext}\;\Varid{x}\;\Varid{y}\;\Varid{s}\;\Varid{t}\;\Varid{a}\;\Varid{b}\mathrel{=}\Conid{OpenGame}\;\{\mskip1.5mu {}\<[E]%
\\
\>[4]{}\hsindent{3}{}\<[7]%
\>[7]{}\Varid{play}\mathbin{::}\Conid{List}\;\Varid{x}\to \Varid{o}\;\Varid{s}\;\Varid{t}\;\Varid{a}\;\Varid{b},{}\<[E]%
\\
\>[4]{}\hsindent{3}{}\<[7]%
\>[7]{}\Varid{evaluate}\mathbin{::}\Conid{List}\;\Varid{x}\to \Varid{c}\;\Varid{x}\;\Varid{s}\;\Varid{y}\;\Varid{r}\to \Conid{List}\;\Varid{y}{}\<[E]%
\\
\>[4]{}\mskip1.5mu\}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
The open-game-engine provides an operator \ensuremath{\Varid{nature}} for instantiating an open game according to a user-defined random draw. Nature corresponds to the terminology used in classical extensive-form Bayesian games, where the first node of the tree is the Nature player providing a type. 
...
\column{7}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\mathbf{data}\;\Conid{UserType}\mathrel{=}\Conid{Normal}\mid \Conid{Adversary}\;\mathbf{deriving}\;(\Conid{Eq},\Conid{Ord},\Conid{Show2}){}\<[E]%
\\[\blanklineskip]%
\>[B]{}\Varid{natureUser}\mathrel{=}{}\<[E]%
\\
\>[B]{}\hsindent{4}{}\<[4]%
\>[4]{}\Varid{nature}\mathbin{\$}{}\<[E]%
\\
\>[4]{}\hsindent{3}{}\<[7]%
\>[7]{}\Varid{distFromList}\;[\mskip1.5mu (\Conid{Adversary},\Varid{threatPrior}),(\Conid{Normal},\mathrm{1}\mathbin{-}\Varid{threatPrior})\mskip1.5mu]{}\<[E]%
\\
\>[4]{}\hsindent{3}{}\<[7]%
\>[7]{}\mathbf{where}\;\Varid{threatPrior}\mathrel{=}\mathrm{0.75}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
\column{7}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\mathbf{data}\;\Conid{StochasticContext}\;\Varid{s}\;\Varid{t}\;\Varid{a}\;\Varid{b}\;\mathbf{where}{}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}\Conid{StochasticContext}\mathbin{::}\Conid{Stochastic}\;(\Varid{z},\Varid{s})\to (\Varid{z}\to \Varid{a}\to \Conid{Stochastic}\;\Varid{b}){}\<[E]%
\\
\>[3]{}\hsindent{4}{}\<[7]%
\>[7]{}\to \Conid{StochasticContext}\;\Varid{s}\;\Varid{t}\;\Varid{a}\;\Varid{b}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
\column{4}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{userGame}\mathbin{::}\Conid{OpenGame}{}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}\Conid{StochasticOptic}{}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}\Conid{StochasticContext}{}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}`[\mskip1.5mu \Conid{Kleisli}\;\Conid{Stochastic}\;\Conid{Signal}\;\Conid{VisitorMove}\mskip1.5mu]{}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}`[\mskip1.5mu [\mskip1.5mu \Conid{DiagnosticInfoBayesian}\;\Conid{Signal}\;\Conid{VisitorMove}\mskip1.5mu]\mskip1.5mu]{}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}(\Conid{Signal},\Conid{Signal}){}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}(){}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}(\Conid{Signal},\Conid{VisitorMove}){}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}((),\Conid{Double}){}\<[E]%
\\
\>[B]{}\Varid{userGame}\mathrel{=}{}\<[E]%
\\
\>[B]{}\hsindent{4}{}\<[4]%
\>[4]{}\Varid{natureUser}\mathbin{>>>}\Varid{copyGame}\mathbin{>>>}{}\<[E]%
\\
\>[B]{}\hsindent{4}{}\<[4]%
\>[4]{}(\Varid{deleteGame}\mathbin{>>>}(\Varid{idGame}\mathbin{\&\&\&}\Varid{dependentDecision}\;\text{\ttfamily \char34 Bob\char34}\;[\mskip1.5mu \Conid{Normal},\Conid{Adversary}\mskip1.5mu])){}\<[E]%
\ColumnHook
\end{hscode}\resethooks
Note that the tensor product of open games concatenates the resultant types into tuples. This eventually means in order to close a game using a tensor produce, we could be working with games that involve types like \ensuremath{((),())} which becomes unwieldy to work with, so we would have to precompose with \ensuremath{\Varid{deleteGame}} which flattens these tuples in the contravariant direction. Otherwise, we can read this expression as a guessing game (like a matching pennies game) where Bob needs to decide if it's encountering an adversary or or a normal user.
\begin{enumerate}
...
The most important function for modellers to use that bakes in Bayesian updating and maximization of the expected payoff after updating their belief is the \ensuremath{\Varid{dependentDecision}} function which implements the selection function given in \cite{BayesianOpenGames}[Definition 4.4.1] for Bayesian agents. 
...
The following example is an open game representation of the well-known Prisoner's Dilemma. For simultaneous play, we would consider the tensor of two utility-maximizing agents each represented by an open game generated by the \ensuremath{\Varid{dependentDecision}} function. In the DSl, we can stack code blocks representing these agents, and pipe their fields together accordingly. The input fields remain blank because neither player makes any observations prior to making their move. Two variables are declared in the outputs fields, \ensuremath{\Varid{decisionPlayer1}} and \ensuremath{\Varid{decisionPlayer2}}, which makes these values accessible to other games. Although the player2 open game is below player1, the Haskell expression in the returns fields have access to both variables \ensuremath{\Varid{decisionPlayer1}} and \ensuremath{\Varid{decisionPlayer2}} to calculate the payoff that each agent recieves. 
\begin{hscode}\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{4}{@{}>{\hspre}l<{\hspost}@{}}%
\column{14}{@{}>{\hspre}l<{\hspost}@{}}%
...
\column{21E}{@{}l@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{myOpenGame}\;\Varid{exoVar1}\;\Varid{exoVar2}\;\Varid{exoVar3}\mathbin{...}\mathrel{=}[\mskip1.5mu \Varid{opengame}\mid {}\<[E]%
\\
\>[B]{}\hsindent{4}{}\<[4]%
\>[4]{}\Varid{inputs}{}\<[14]%
\>[14]{}\mathbin{:}{}\<[21]%
...
\\
\>[B]{}\hsindent{4}{}\<[4]%
\>[4]{}\Varid{operation}\mathbin{:}\Varid{dependentDecision}\;\text{\ttfamily \char34 player1\char34}\;(\Varid{const}\;[\mskip1.5mu \Conid{Cooperate},\Conid{Defect}\mskip1.5mu]);{}\<[E]%
\\
\>[B]{}\hsindent{4}{}\<[4]%
\>[4]{}\Varid{outputs}{}\<[14]%
\>[14]{}\mathbin{:}\Varid{decisionPlayer1};{}\<[E]%
\\
\>[B]{}\hsindent{4}{}\<[4]%
\>[4]{}\Varid{returns}{}\<[14]%
\>[14]{}\mathbin{:}\Varid{prisonersDilemmaMatrix}\;\Varid{decisionPlayer1}\;\Varid{decisionPlayer2};{}\<[E]%
\\[\blanklineskip]%
\>[B]{}\hsindent{4}{}\<[4]%
\>[4]{}\Varid{inputs}{}\<[14]%
\>[14]{}\mathbin{:}{}\<[21]%
...
\\
\>[B]{}\hsindent{4}{}\<[4]%
\>[4]{}\Varid{operation}\mathbin{:}\Varid{dependentDecision}\;\text{\ttfamily \char34 player2\char34}\;(\Varid{const}\;[\mskip1.5mu \Conid{Cooperate},\Conid{Defect}\mskip1.5mu]);{}\<[E]%
\\
\>[B]{}\hsindent{4}{}\<[4]%
\>[4]{}\Varid{outputs}{}\<[14]%
\>[14]{}\mathbin{:}\Varid{decisionPlayer2};{}\<[E]%
\\
\>[B]{}\hsindent{4}{}\<[4]%
\>[4]{}\Varid{returns}{}\<[14]%
\>[14]{}\mathbin{:}\Varid{prisonersDilemmaMatrix}\;\Varid{decisionPlayer2}\;\Varid{decisionPlayer1};{}\<[E]%
\\[\blanklineskip]%
\>[B]{}\hsindent{4}{}\<[4]%
\>[4]{}\mathbin{:----------------------------:}{}\<[E]%
\\[\blanklineskip]%
...
Here, \ensuremath{[\mskip1.5mu \Varid{opengame}}...\ensuremath{\mskip1.5mu]} is TemplateHaskell's quotation syntax, which returns the \ensuremath{\Conid{OpenGame}} data type. The syntax is divided into two sections, (1) the external components and (2) the internals of the open game. The external components are comprised of the two fields \ensuremath{\Varid{inputs}} and \ensuremath{\Varid{feedback}} above the delimiter \ensuremath{\mathbin{:----:}}, and \ensuremath{\Varid{outputs}} and \ensuremath{\Varid{returns}} at the bottom. As introduced before in the formal definition of open games, these fields correspond to the pairs of objects corresponding to the type of the open game, with \ensuremath{\Varid{inputs}} and \ensuremath{\Varid{feedback}} as the pair $(S,T)$ amd \ensuremath{\Varid{outputs}} and \ensuremath{\Varid{returns}} as $(A, B)$. These fields designate the variables available for use, and the values produced by the internals of the game. Note that user-declared variables are in scope for the internals of the game and correspond with the \ensuremath{\Varid{inputs}} and \ensuremath{\Varid{returns}} fields. On the other hand, the remaining two fields designate the values that will be generated by the internals of the game to be passed to other games. As in the above example, the variables declared in the outputs field can be used in any of the inputs or returns fields.


We treat any parameters such as \ensuremath{\Varid{exoVar}} as exogenous parameters introduced outside the game. This ends up being an appropriate place to pipeline the infrastructure required to characterize payoff functions, parameterize strategies, and other techniques in order to keep the DSL syntax generic. See chapter 2 for the infrastructure and coding patterns used to best utilize this concept.  

Threading these variables together through these fields is up to the discretion of the modeller. An open game can completely change with a small change, for example puting \ensuremath{\Varid{decisionPlayer1}} into the inputs field of the \ensuremath{\Varid{player2}} code block. In this representation, \ensuremath{\Varid{player1}} moves first and \ensuremath{\Varid{player2}} will be able to observe the output. The compiler will be responsible for making sure the types line up accordingly, but matching the modeller's intention to what's being written as an open game is the modeller's responsibility.

There is some assistance from the type-checker, however, to make sure that dependencies are consistent when making changes like this. By exposing the first player's decision to the second, the model execution code should fail to compile because now the strategy of player 2 is dependent on this variable. The type-checker will follow a top-down ordering when typing, so when supplying a list of strategies to run this game, this should be a list ordered left-to-right lining up with the code blocks. We can read this from the type signature for each game, where the \ensuremath{`[\mskip1.5mu \mskip1.5mu]} constructor in the type parameters exactly corresponds with the strategies the game is expecting for each of the code blocks. We can also see that the final four parameters are \ensuremath{()}, which designates a closed game. 
\begin{minipage}\textwidth
\begin{hscode}\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{3}{@{}>{\hspre}l<{\hspost}@{}}%
\column{4}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{myOpenGame}\mathbin{::}\Conid{OpenGame}{}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}\Conid{StochasticOptic}{}\<[E]%
\\
...
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}`[\mskip1.5mu \Conid{Kleisli}\;\Conid{Stochastic}\;()\;\Conid{PrisonerMove},{}\<[E]%
\\
\>[3]{}\hsindent{1}{}\<[4]%
\>[4]{}\Conid{Kleisli}\;\Conid{Stochastic}\;()\;\Conid{PrisonerMove}\mskip1.5mu]{}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}`[\mskip1.5mu [\mskip1.5mu \Conid{DiagnosticInfoBayesian}\;()\;\Conid{PrisonerMove}\mskip1.5mu],{}\<[E]%
\\
\>[3]{}\hsindent{1}{}\<[4]%
\>[4]{}[\mskip1.5mu \Conid{DiagnosticInfoBayesian}\;()\;\Conid{PrisonerMove}\mskip1.5mu]\mskip1.5mu]{}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}(){}\<[E]%
\\
...
The \textit{behavioural strategies} of type \ensuremath{\Conid{Kleisli}\;\Conid{Stochastic}\;\Varid{s}\;\Varid{a}} have already been described in a previous section, but we made no indication for what \ensuremath{\Varid{s}} and \ensuremath{\Varid{a}} had to be. The 
...
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\mathbf{type}\;\Conid{PayoffReader}\;\Varid{a}\mathrel{=}\Conid{Reader}\;\Varid{a}\;\Conid{Double}{}\<[E]%
\\[\blanklineskip]%
...
\>[4]{}\Varid{returns}\mathbin{:};{}\<[E]%
\\[\blanklineskip]%
\>[B]{}\mid \mskip1.5mu]\;\mathbf{where}\;\Varid{calculateCurrAttackerPayoff}\;\Varid{x}\;\Varid{y}\;\Varid{z}\mathrel{=}\Varid{runPayoff}\;\Varid{params}\;(\Varid{payoffReader1}\;\Varid{x}\;\Varid{y}\;\Varid{z}){}\<[E]%
\\
\>[B]{}\hsindent{11}{}\<[11]%
