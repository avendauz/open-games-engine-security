...
equilibriumChecks = do 
    describe "IDSA equilibrium checks" $ do 
        it "is in equilibrium with a mixed strategy" $  -- checking expected equilibrium
            do generateEquilibrium $ evaluate (idsAGame exampleData) (totalGameStrategies exampleData) (instantiateContext visitorPayoff defenderPayoff exampleData)